...
# 创建输出目录并初始化日志记录器（logger 和 SummaryWriter）
##### ---- Logger ---- #####
logger = utils_model.get_logger(args.out_dir)
writer = SummaryWriter(args.out_dir)
logger.info(json.dumps(vars(args), indent=4, sort_keys=True))

# 使用 WordVectorizer 加载 GloVe 词向量模型，将文本转化为固定维度的特征表示
from utils.word_vectorizer import WordVectorizer
w_vectorizer = WordVectorizer('./glove', 'our_vab')

# dataset_TM_eval.DATALoader: 数据加载器，将文本与动作数据加载为批量数据（batch）
...
# 加载 CLIP 模型（ViT-B/32），用于将文本编码为固定维度的特征向量
'''
Clip作用：
1. CLIP 在文本到动作任务中负责将文本编码为嵌入向量。
2. CLIP 特征会在动作生成后用于计算 Matching 分数。
'''
...
# 加载 VQ-VAE 模型，负责将动作数据编码为离散 token，并在生成时将 token 解码为动作
...
# 加载 GPT 模型，用于将文本特征映射为动作 token 序列
...
# 从 args.resume_pth 加载预训练的 VQ-VAE 模型权重
...
生成动作
作用：
1. 将文本特征输入 Transformer 模型。
2. 生成动作 token 并解码为连续动作数据。
3. 将生成的动作保存为 .npy 文件。
