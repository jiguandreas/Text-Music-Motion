# 混合加载music和text数据进行transformer训练
import os 
import torch
import numpy as np
...
from dataset import dataset_TM_MM_train
...
# 添加两个线性层来处理不同模态的特征
# text_linear = torch.nn.Linear(args.clip_dim, args.embed_dim_gpt).cuda()
# music_linear = torch.nn.Linear(args.clip_dim, args.embed_dim_gpt).cuda()  # 假设music特征维度也是clip_dim，如果不是请调整
# 修改线性层定义，确保输出维度与模型期望匹配
text_linear = torch.nn.Linear(args.clip_dim, 512).cuda()  # 改为模型期望的维度
music_linear = torch.nn.Linear(args.clip_dim, 512).cuda()
...
##### ---- Optimizer & Scheduler ---- #####
# optimizer = utils_model.initial_optim(args.decay_option, args.lr, args.weight_decay, [trans_encoder, text_linear, music_linear], args.optimizer)
# all_parameters = list(trans_encoder.parameters()) + list(text_linear.parameters()) + list(music_linear.parameters())
# optimizer = utils_model.initial_optim(args.decay_option, args.lr, args.weight_decay, 
#                                     all_parameters,  # 直接传递参数列表，不要封装在字典中
#                                     args.optimizer)
...
if args.optimizer == "adamw":
    optimizer = torch.optim.AdamW(all_parameters, lr=args.lr, betas=(0.5, 0.9), weight_decay=args.weight_decay)
elif args.optimizer == "adam":
    optimizer = torch.optim.Adam(all_parameters, lr=args.lr, betas=(0.5, 0.9), weight_decay=args.weight_decay)
else:
    # 添加其他优化器的情况
...
##### ---- get code ---- #####
# 这里是获取motion 的code，即motion_token用于Transformer的输入
...
# 混合加载music和text数据
train_loader = dataset_TM_MM_train.DATALoader(args.dataname, args.batch_size, args.nb_code, args.vq_name, unit_length=2**args.down_t)
train_loader_iter = dataset_TM_MM_train.cycle(train_loader)
...
while nb_iter <= args.total_iter:
    ...
    modalities, modality_data_list, m_tokens, m_tokens_len = batch
    ...
    for modality, data in zip(modalities, modality_data_list):
        if modality == "text":
            print(222)
            text = clip.tokenize([data], truncate=True).cuda()
            feat = clip_model.encode_text(text).float()
            feat = text_linear(feat)
    ...
    # 模态通用的 Transformer 输入
    cls_pred = trans_encoder(a_indices, input_feature)
    ...
    nb_sample_train += (m_tokens_len + 1).sum().item()
    nb_iter += 1
