# 独立开来训练，music和text模态分别load数据
# 新增了一个参数来确定输入的模态是text还是music
import os 
import torch
import numpy as np
...
from dataset import dataset_MM_train
from dataset import dataset_TM_eval
from dataset import dataset_tokenize
from dataset import dataset_music_tokenize
...
##### ---- Dataloader ---- #####
if args.input_modality == 'text':
    train_loader_token = dataset_tokenize.DATALoader(args.dataname, 1, unit_length=2**args.down_t)
else:  # music
    train_loader_token = dataset_music_tokenize.DATALoader(args.dataname, 1, unit_length=2**args.down_t)
...
if args.input_modality == 'text':
    ...
    train_loader = dataset_MM_train.DATALoader(args.dataname, args.batch_size, args.nb_code, args.vq_name, music_data_path='/disk2/yuyusun/TM2M-GPT/dataset/HumanML3D/music', unit_length=2**args.down_t)
    train_loader_iter = dataset_MM_train.cycle(train_loader)
...
while nb_iter <= args.total_iter:
    ...
    if args.input_modality == 'text':
        ...
        input_feature = feat_clip_text

    else:  # music
        music_seq, m_tokens, m_tokens_len = batch
        m_tokens, m_tokens_len = m_tokens.cuda(), m_tokens_len.cuda()
        bs = m_tokens.shape[0]
        target = m_tokens.cuda()
        input_feature = music_seq.cuda().float()
