...
def custom_collate_fn(batch):
    modalities, inputs, m_tokens, m_tokens_len = zip(*batch)

    # 转 tensor
    m_tokens = torch.tensor(m_tokens, dtype=torch.long)
    m_tokens_len = torch.tensor(m_tokens_len, dtype=torch.long)

    # 如果是 numpy 类型的 music_seq，也转成 tensor，统一形状
    processed_inputs = []
    for mod, data in zip(modalities, inputs):
        if mod == "music":
    ...
    return list(modalities), processed_inputs, m_tokens, m_tokens_len
...
class Text2MotionDataset(data.Dataset):
        ...
        for name in tqdm(id_list):
            if name in self.music_id_set:
                try:
                    ...
                    music_seq = np.load(pjoin(self.music_dir, f'{name}.npy'))
    ...
    def __getitem__(self, item):
        ...
        if name in self.music_id_set:
            ...
            return "music", music_seq, m_tokens.reshape(-1), m_tokens_len

        else:
            m_token_list, text_list = data['m_token_list'], data['text']
            m_tokens = random.choice(m_token_list)
            caption = random.choice(text_list)['caption']
            ...
            return "text", caption, m_tokens.reshape(-1), m_tokens_len
